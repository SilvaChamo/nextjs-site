...
export function NewsletterCard() {
    ...
    return (
        <div className="bg-slate-900 rounded-[15px] p-8 text-white relative overflow-hidden group border border-white/5">
                    ...
                    <form onSubmit={handleSubscribe} className="space-y-3 pt-2">
                        <div className="relative group">
                            <input
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-[12px] px-5 py-4 text-xs font-medium focus:outline-none focus:border-[#f97316] focus:bg-white/10 transition-all placeholder:text-slate-600"
                                required
                            />
                        </div>
                        <button
                            disabled={status === 'loading'}
                            className="w-full bg-[#f97316] hover:bg-orange-600 disabled:opacity-50 text-white font-black text-[10px] uppercase tracking-[0.2em] py-4 rounded-[12px] shadow-lg shadow-orange-500/20 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
                        >
