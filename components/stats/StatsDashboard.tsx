...
export function StatsDashboard({ slug }: StatsDashboardProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // 'producao', 'economia', etc matches DB category
            ...
            if (error) {
                ...
                // Map DB label -> name to match component expectance
                const formattedData = stats.map(s => ({
                    name: s.label,
                    value: s.value,
                    variation: s.variation
                }));
                // If Manica/Production, we might need manual sorting? The DB returns arbitrarily.
                // Assuming order is roughly fine or handled by Chart.
